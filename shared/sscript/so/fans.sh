...
if [ ! -f "$RUTA_EC" ]; then
    ...
    # Verificación estricta para Arch / Kernels 5.15+
    sleep 1
    if [ ! -f "$RUTA_EC" ]; then
        echo "🚨 ERROR CRÍTICO: El kernel ha bloqueado el acceso a ec_sys por seguridad."
        echo "Para que el control de ventiladores funcione en Arch Linux debes añadir:"
        echo "  ec_sys.write_support=1"
        echo "a los parámetros de tu kernel en GRUB (archivo /etc/default/grub, en GRUB_CMDLINE_LINUX_DEFAULT)."
        echo "Luego ejecuta: sudo grub-mkconfig -o /boot/grub/grub.cfg y reinicia."
        exit 1
    fi
fi
