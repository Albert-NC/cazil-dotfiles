...
REQUIRED_FILES=("facer.c" "Makefile" "dkms.conf" "nitro_rgb.py" "facer_rgb.py")
MISSING=0
...
mkdir -p "$DKMS_SRC"
cp "$SCRIPT_SRC_DIR/facer.c"   "$DKMS_SRC/"
cp "$SCRIPT_SRC_DIR/Makefile"  "$DKMS_SRC/"
cp "$SCRIPT_SRC_DIR/dkms.conf" "$DKMS_SRC/"
...
mkdir -p "$INSTALL_DIR"
cp "$SCRIPT_SRC_DIR/nitro_rgb.py"  "$INSTALL_DIR/"
cp "$SCRIPT_SRC_DIR/facer_rgb.py"  "$INSTALL_DIR/battery_monitor.sh"
chmod +x "$INSTALL_DIR/battery_monitor.sh" "$INSTALL_DIR/nitro_rgb.py"
...
echo ""
echo "  Probar luces:"
echo "    python3 $INSTALL_DIR/nitro_rgb.py --all -cR 0 -cG 255 -cB 0"
echo ""
